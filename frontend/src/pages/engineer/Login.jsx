  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center p-6 bg-slate-950 overflow-hidden font-sans">
      ...
      >
        ...
        <button 
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-slate-500 hover:text-cyan-600 transition-smooth mb-8 font-bold text-xs tracking-wider"
        >
          <ArrowLeft size={16} /> BACK TO PORTAL
        </button>

        <div className="mb-8 text-center">
          <div className="bg-cyan-500/10 w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4 text-cyan-600">
            <Wrench size={36} />
          </div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">ENGINEER LOGIN</h2>
          <p className="text-xs text-slate-500 mt-1 font-medium">Operations Dashboard Access</p>
        </div>

        <form className="space-y-5" onSubmit={handleLogin}>
          {/* Employee ID Field */}
          <div>
            <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5">EMPLOYEE ID</label>
            <div className="relative">
              ...
              <input
                ...
                required
                className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 outline-none text-sm text-slate-800 font-medium transition-smooth"
                placeholder="enae1"
              />
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full py-4 bg-slate-900 hover:bg-cyan-600 text-white rounded-xl font-bold text-xs tracking-widest uppercase transition-smooth shadow-lg hover:shadow-cyan-500/20 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none mt-2"
          >
            {isSubmitting ? 'VERIFYING...' : 'ENGINEER SECURE LOGIN'}
          </button>
        </form>

        <p className="mt-8 text-center text-xs text-slate-400 font-semibold tracking-wider uppercase">
          Restricted access for field engineers only.
        </p>
      </motion.div>
    </div>
  );
